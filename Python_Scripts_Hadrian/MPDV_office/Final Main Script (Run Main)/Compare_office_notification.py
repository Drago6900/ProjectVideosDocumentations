...
mailjet_client = MailjetClient(auth=(MAILJET_API_KEY, MAILJET_SECRET_KEY), version='v3.1')


# FaceSec API endpoints, PLEASE CHANGE IP in the links below to match FaceSec IP
FACESEC_LOGIN_URL = "http://172.17.170.111:8081/api/account/login?password=25802580"
FACESEC_PERSONS_URL = "http://172.17.170.111:8081/api/persons?size=100&page=0"
FACESEC_RULES_URL = "http://172.17.170.111:8081/api/rules"
FACESEC_ADD_PERSONNEL_URL = "http://172.17.170.111:8081/api/person"
FACESEC_UPLOAD_IMAGE_URL = "http://172.17.170.1114242:8081/api/upload/image"
...
# The newly implemented functions for the FaceSec API, email and sms notifcation, and a logout function.
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            try:
                ...
                print("Raw rules data:", rules_data)  # Print raw data to inspect
                ...
                print(f"Error processing rules data: {e}")
        else:
    ...
    try:
        with open(image_file_path, 'rb') as file:
            ...
            if response.status_code == 200:
                ...
                print("Image uploaded successfully. Response data:", uploaded_image_data)
                return uploaded_image_data
            else:
...
# Adding personnel to facsec device
def add_personnel_to_facesec(cookies, personnel_data):
    if not personnel_data:
        print("No personnel data provided.")
        return

    try:
        print("Starting to add personnel to FaceSec...")
        for person in personnel_data['content']:
            ...
            response = requests.post(FACESEC_ADD_PERSONNEL_URL, json=payload, cookies=cookies)
            print("Response received:", response.status_code, response.text)
            if response.status_code == 200:
                print(f"Successfully added personnel {person['name']} to FaceSec.")
            elif response.status_code == 400 and response.json().get('message') == 'uuid_repeat':
                print(f"Skipping addition of existing personnel {person['name']}")
            else:
                print(f"Failed to add personnel {person['name']} to FaceSec. Status code: {response.status_code}, Response: {response.text}")

    except requests.RequestException as e:
        print(f"Error adding personnel to FaceSec: {e}")

# Getting user info from Hydra Server
def get_user_info_from_hydra():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"Failed to retrieve user information from HYDRA. Status code: {response.status_code}, Response: {response.text}")
    except requests.RequestException as e:
...
def compare_data(facesec_data, hydra_data):
    print("Facesec data type:", type(facesec_data))
    print("Hydra data type:", type(hydra_data))

    if isinstance(facesec_data, dict) and 'entity' in facesec_data:
        facesec_dict = {(entry['no'], entry['name']) for entry in facesec_data['entity']['content']}
    else:
        print("Invalid facesec data format")
        return None

    if isinstance(hydra_data, list):
        hydra_dict = {(entry['data'][1], entry['data'][0]) for entry in hydra_data[1:]}
    else:
        print("Invalid hydra data format")
        return None

    matching_users = facesec_dict.intersection(hydra_dict)
    non_matching_hydra = hydra_dict - facesec_dict

    # Sort the non-matching Hydra users by their number
    sorted_non_matching_hydra = sorted(non_matching_hydra, key=lambda x: x[0])
    ...
    comparison_result = {
        ...
        "non_matching_hydra": [
            {"no": name, "name": no}
            for name, no in sorted_non_matching_hydra
        ]
    }
...
# The function used in Main_office: compare_and_sync
